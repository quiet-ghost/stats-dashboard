...
function App() {
  ...
  React.useEffect(() => {
    ...
    const combinedData = combineDataFromFiles(
      completedFiles.map((f) => ({ data: f.data! })),
    );
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              <h1 className="text-4xl font-bold mb-2">
                Pick & (not yet) Pack Stats Dashboard
              </h1>
              <p className="text-muted-foreground">
                Upload Excel files to analyze warehouse performance data. Data
                is processed locally and creates table view for accurate
                employee analysis.
              </p>
            </div>
            ...
            {allData.length > 0 && (
              <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
                <p className="text-green-800 dark:text-green-200 font-medium">
                  ✓ Successfully processed {allData.length} records from{" "}
                  {files.filter((f) => f.status === "completed").length} files
                </p>
                <p className="text-green-600 dark:text-green-300 text-sm mt-1">
                  Switch to the Dashboard tab to view your performance
                  analytics.
                </p>
              </div>
            )}
